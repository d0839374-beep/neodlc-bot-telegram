...
// Все значения — только из переменных окружения Amvera
const BOT_TOKEN = process.env.BOT_TOKEN;
const DISCORD_URL = process.env.DISCORD_URL;
const DOWNLOAD_URL = process.env.DOWNLOAD_URL;
const STATUS_TEXT = process.env.STATUS_TEXT;  // НИКАКОГО значения по умолчанию!

// Проверка обязательных переменных
if (!BOT_TOKEN || !DISCORD_URL || !DOWNLOAD_URL || !STATUS_TEXT) {
    console.error('Ошибка: BOT_TOKEN, DISCORD_URL, DOWNLOAD_URL, STATUS_TEXT должны быть заданы в переменных окружения Amvera');
    process.exit(1);
}
...
console.log('Бот запущен (polling). Все настройки из окружения Amvera.');